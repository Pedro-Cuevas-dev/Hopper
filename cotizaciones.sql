-- --------------------------------------------------------
-- Host:                         127.0.0.1
-- Versión del servidor:         8.0.30 - MySQL Community Server - GPL
-- SO del servidor:              Win64
-- HeidiSQL Versión:             12.1.0.6537
-- --------------------------------------------------------

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET NAMES utf8 */;
/*!50503 SET NAMES utf8mb4 */;
/*!40103 SET @OLD_TIME_ZONE=@@TIME_ZONE */;
/*!40103 SET TIME_ZONE='+00:00' */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;


-- Volcando estructura de base de datos para cotizaciones
CREATE DATABASE IF NOT EXISTS `cotizaciones` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `cotizaciones`;

-- Volcando estructura para tabla cotizaciones.cliente
CREATE TABLE IF NOT EXISTS `cliente` (
  `id_client` int NOT NULL,
  `Nombre` varchar(50) NOT NULL,
  `Apellido` varchar(50) NOT NULL,
  `Correo` varchar(50) NOT NULL,
  `Telefono` char(20) DEFAULT NULL,
  `Direccion` varchar(50) NOT NULL DEFAULT '0',
  `Num de Tarjeta` varchar(50) NOT NULL DEFAULT '0',
  `CVV` int NOT NULL DEFAULT '0',
  `Expiracion` varchar(5) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id_client`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Volcando datos para la tabla cotizaciones.cliente: ~0 rows (aproximadamente)
INSERT IGNORE INTO `cliente` (`id_client`, `Nombre`, `Apellido`, `Correo`, `Telefono`, `Direccion`, `Num de Tarjeta`, `CVV`, `Expiracion`) VALUES
	(1, 'Raymundo', 'Apellido', 'Prueba@gmail.com', '6181276565', 'morelos 218', '0', 0, '0');

-- Volcando estructura para tabla cotizaciones.usuarios
CREATE TABLE IF NOT EXISTS `usuarios` (
  `usuario` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL,
  `servidor` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL,
  `contra` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL,
  `base_datos` int DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Volcando datos para la tabla cotizaciones.usuarios: ~1 rows (aproximadamente)
INSERT IGNORE INTO `usuarios` (`usuario`, `servidor`, `contra`, `base_datos`) VALUES
	('ray', 'mirawey12@gmail.com', 'sopafria23', NULL);

/*!40103 SET TIME_ZONE=IFNULL(@OLD_TIME_ZONE, 'system') */;
/*!40101 SET SQL_MODE=IFNULL(@OLD_SQL_MODE, '') */;
/*!40014 SET FOREIGN_KEY_CHECKS=IFNULL(@OLD_FOREIGN_KEY_CHECKS, 1) */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40111 SET SQL_NOTES=IFNULL(@OLD_SQL_NOTES, 1) */;
